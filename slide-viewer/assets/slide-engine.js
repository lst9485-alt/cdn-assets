// 스케일
  ...
  // 태그칩 순차 팝: tag-group 내 .tag-chip에 staggered transition-delay
  ...
  // 카드 순차 등장: 같은 step-layer 내 여러 카드에 staggered transition-delay
  ...
  function buildLineChart(el) {
    ...
    const svgHTML = `<svg viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
      ...
      <circle cx="${lastPt.x}" cy="${lastPt.y}" r="6" fill="#4A90D9"/>
      ...
      <circle class="lc-travel-dot" r="8" fill="#4A90D9" filter="url(#lcGlow)"><animateMotion dur="4s" repeatCount="indefinite" path="${pathD}" begin="5.5s"/></circle>
    </svg>`;
  ...
  document.addEventListener('keydown', e => {
    ...
    if ((e.ctrlKey || e.metaKey) && e.code === 'KeyG' && editMode) {
      ...
      if (allSameGroup) {
        ...
        els.forEach(el => {
          ...
          if (srcLayer && parseInt(srcLayer.dataset.step) !== 0) {
            ...
            if (layer0) {
              ...
              if (!srcLayer.querySelector(EDITABLE_SEL)) { srcLayer.remove(); recalcSteps(slide); }
            }
          }
        });
      } else {
        // 기존 그룹 해제 후 새 그룹 생성 + step-1로 이동
        selectedEls.forEach(el => delete el.dataset.group);
        groupCounter++;
        ...
        els.forEach(el => {
          ...
          const srcLayer = el.closest('.step-layer');
          if (srcLayer && parseInt(srcLayer.dataset.step) === 0) {
            let layer1 = slide.querySelector('.step-layer[data-step="1"]');
            if (!layer1) {
              layer1 = document.createElement('div');
              layer1.className = 'step-layer';
              layer1.dataset.step = '1';
              layer1.innerHTML = '<div class="step-dim"></div>';
              slide.appendChild(layer1);
            }
            srcLayer.removeChild(el);
            if (el.matches('.text-area')) { const sc = document.createElement('div'); sc.className = 'step-content'; sc.innerHTML = el.innerHTML; el.innerHTML = ''; el.appendChild(sc); }
    ...
    if ((e.code === 'Delete' || e.code === 'Backspace') && editMode) {
      ...
      [...selectedEls].forEach(el => {
        ...
        if (parseInt(layer.dataset.step) > 0 && !layer.querySelector(EDITABLE_SEL)) {
          layer.remove();
          recalcSteps(slide);
        }
      });
...
// D키: 요소 선택 중이면 오버레이 추가, 미선택이면 dim 표시/숨김 토글 (편집 모드)
  ...
  function getCleanHTML() {
    ...
    document.querySelectorAll('.child-selected').forEach(c => c.classList.remove('child-selected'));
  ...
  document.addEventListener('mousedown', e => {
    ...
    if (e.target.classList.contains('resize-handle')) {
      ...
      // edge handle: 단방향 리사이즈 (slide-el, img, emoji-icon 전용)
      if (resizeEdge && (selectedEl.tagName === 'IMG' || selectedEl.classList.contains('emoji-icon') || selectedEl.classList.contains('slide-el'))) {
      ...
      // IMG / emoji-icon 리사이즈 (4모서리, 비율 유지)
      if (selectedEl.tagName === 'IMG' || selectedEl.classList.contains('emoji-icon') || selectedEl.classList.contains('slide-el')) {
        resizeInitFontSizes = null;
        resizeImgInit = null;
  ...
  function buildAnimationTab(list, header) {
    ...
    // header: "클릭 시 표시" 토글 (항상 표시, 요소 미선택 시 disabled)
  ...
  function moveToStep(el, targetStep) {
    ...
    if (targetStep >= 1) {
      ...
      if (parseInt(srcLayer.dataset.step) > 0 && !srcLayer.querySelector(EDITABLE_SEL)) {
        srcLayer.remove();
        recalcSteps(slide);
      }
    } else {
      ...
      layer0.appendChild(el);
      if (!srcLayer.querySelector(EDITABLE_SEL)) {
        srcLayer.remove();
        recalcSteps(slide);
      }
    }
