...
// D키: 요소 선택 중이면 오버레이 추가, 미선택이면 dim 표시/숨김 토글 (편집 모드)
  ...
  function ensureOverviewNotesPanel() {
    ...
    panel.innerHTML = `
      ...
      <div class="ov-notes-hint">대본 보기 켜짐: 썸네일 클릭 시 미리보기</div>
  ...
  function insertOverviewSlideAt(sourceIdx, targetIdx = currentSlide + 1) {
    if (!(document.body && document.body.dataset.generated === 'true')) {
      if (typeof showToast === 'function') showToast('에디터에서는 슬라이드 삽입을 막았습니다.');
      return;
    }
    const container = document.getElementById('stage');
    ...
    const clone = source.cloneNode(true);
    const origId = clone.dataset.slideId || clone.id || '';
    clone.dataset.slideId = origId + '_insert_' + Date.now();
    clone.id = '';
    clone.classList.remove('active');
    const boundedTarget = Math.max(0, Math.min(targetIdx, slides.length));
    const anchor = slides[boundedTarget] || null;
    if (anchor) container.insertBefore(clone, anchor);
    else container.appendChild(clone);
    slides = [...container.querySelectorAll(':scope > .slide')];
    rebuildSlidesByKey();
    currentSlide = Math.min(boundedTarget, slides.length - 1);
    buildFilmstrip();
    buildOverview();
    if (typeof buildSlideJumpNav === 'function') buildSlideJumpNav();
    closeOverview();
  ...
  function buildOverview() {
    ...
    slides.forEach((slide, slideIdx) => {
      ...
      if (!document.body.classList.contains('frozen-legacy-deck') && document.body && document.body.dataset.generated === 'true') {
        ...
        insertBtn.className = 'ov-action-btn';
        insertBtn.textContent = '삽입';
        insertBtn.addEventListener('click', (e) => {
          e.stopPropagation();
          insertOverviewSlideAt(slideIdx);
        });
        const copyBtn = document.createElement('button');
        copyBtn.type = 'button';
        copyBtn.className = 'ov-action-btn';
        ...
        actions.append(insertBtn, copyBtn, delBtn);
  ...
  function showSlideContextMenu(x, y, idx) {
    ...
    insertBtn.textContent = '➕ 현재 뒤에 삽입';
    insertBtn.style.cssText = 'padding:8px 16px;cursor:pointer;';
    insertBtn.addEventListener('mouseenter', () => insertBtn.style.background = '#444');
    insertBtn.addEventListener('mouseleave', () => insertBtn.style.background = 'none');
    insertBtn.addEventListener('click', () => {
      try {
        hideSlideContextMenu();
        insertOverviewSlideAt(idx);
      } catch (err) {
        if (typeof showToast === 'function') showToast('삽입 오류: ' + err.message, 5000);
      }
    });
  ...
  const PG_TO_TYPE_META = {
    ...
    5: {"label":"T05 번호항목","schemaRequiredCount":2,"fillRange":[2,6],"itemsRange":[1,5],"displaySteps":null,"usageScope":null,"media":{"image":false,"imageRequired":false,"emoji":false}},
    6: {"label":"T06 체크리스트","schemaRequiredCount":2,"fillRange":[2,6],"itemsRange":[1,5],"displaySteps":null,"usageScope":null,"media":{"image":false,"imageRequired":false,"emoji":false}},
    ...
    10: {"label":"T10 아이콘플로우","schemaRequiredCount":3,"fillRange":[3,11],"itemsRange":[1,5],"displaySteps":null,"usageScope":null,"media":{"image":false,"imageRequired":false,"emoji":true}},
  ...
  function typeNumberOfMeta(meta) {
    ...
    const match = String(meta.label).match(/^T(\d+)/);
  ...
  function inputLabelOfSlide(slide, meta) {
    ...
    let count = 0;
    if ([3, 4, 26].includes(pg)) count = countBySelector(slide, '.step-layer[data-step]:not([data-step="0"]) .grid-card, .step-layer[data-step]:not([data-step="0"]) .card');
    else if ([5, 6].includes(pg)) count = countBySelector(slide, '.step-layer[data-step]:not([data-step="0"]) .num-item, .step-layer[data-step]:not([data-step="0"]) .check-item');
    else if (pg === 7) count = countBySelector(slide, '.step-layer[data-step]:not([data-step="0"]) [data-type="아이콘행"]');
    else if (pg === 8) count = countBySelector(slide, '.step-layer[data-step]:not([data-step="0"]) .icon-item');
    else if (pg === 9 || pg === 15) count = countBySelector(slide, '.step-layer[data-step]:not([data-step="0"]) .flow-box');
    else if (pg === 10) count = countBySelector(slide, '.step-layer[data-step]:not([data-step="0"]) .icon-flow-item');
    else if (pg === 11) count = countBySelector(slide, '.bar-row, .hbar-row');
    else if (pg === 16) count = countBySelector(slide, '.step-layer[data-step]:not([data-step="0"]) .step-timeline');
    else if ([17, 24].includes(pg)) count = countBySelector(slide, '.compare-col.good .compare-item');
    else if (pg === 19) count = countBySelector(slide, '.alert-banner');
    else if (pg === 20) count = countBySelector(slide, '.tag-chip');
    else if (pg === 21) count = countBySelector(slide, '.section-badge');
    else if (pg === 22) count = countBySelector(slide, '.btn-pill');
    else if (pg === 25) count = countBySelector(slide, '.bullet-card');
    else if (pg === 28) count = countBySelector(slide, '.comp-table tbody tr');
    else if (pg === 37) count = countBySelector(slide, '.step-layer[data-step]:not([data-step="0"]) .postit');
    else if (pg === 38) count = countBySelector(slide, '.step-layer[data-step]:not([data-step="0"]) .postit-mid');

    if (pg === 11 && count) return String(1 + (count * 2));
    if (pg === 12) return '5';
    if (pg === 13) return '3+';
    if ([15, 25, 37].includes(pg) && count) return String(1 + count);
    if ([16, 17, 24, 38].includes(pg) && count) return String(1 + (count * 2));
    if (pg === 28 && count) return String(1 + (count * 3) + 1);
    if (count) return String(1 + count);
    if (meta && Array.isArray(meta.fillRange)) {
      const minFill = meta.fillRange[0];
      const maxFill = meta.fillRange[1];
      return minFill === maxFill ? String(minFill) : `${minFill}~${maxFill}`;
    }
    return null;
  }
  ...
  function createTypeMetaChips(slide, options) {
    ...
    const addChip = (label, title, extraClass) => {
      if (!label) return;
  ...
  function syncTypeMetaChips() {
    ...
    document.querySelectorAll('#overview-grid .ov-item').forEach(item => {
      ...
      if (!slide) return;
      item.querySelector(':scope > .type-meta-chips')?.remove();
      item.appendChild(createTypeMetaChips(slide));
    });
    document.querySelectorAll('#filmstrip-inner .fs-item').forEach(item => {
      const slide = slides[parseInt(item.dataset.canonicalIdx || '-1', 10)];
      if (!slide) return;
      item.querySelector(':scope > .type-meta-chips')?.remove();
  ...
  function wrapTypeMetaBuilder(name) {
    const original = globalThis[name];
    if (typeof original !== 'function' || original.__typeMetaWrapped) return;
    const wrapped = function() {
      const result = original.apply(this, arguments);
      syncTypeMetaChips();
      return result;
    };
    wrapped.__typeMetaWrapped = true;
    globalThis[name] = wrapped;
  }

  function collectAnimationRows(slide) {
    const rows = [];
    const seenGroups = new Set();
    slide.querySelectorAll('.step-layer').forEach(layer => {
      const step = parseInt(layer.dataset.step, 10);
      if (step === 0) return;
      getOrderedEls(layer).forEach(el => {
        if (el.classList.contains('step-dim')) return;
        const gid = el.dataset.group;
        if (gid) {
          ...
          seenGroups.add(key);
          rows.push({ step, el, gid, members: [...layer.querySelectorAll(`[data-group="${CSS.escape(gid)}"]`)] });
          return;
        }
        rows.push({ step, el });
      });
    ...
    sorted.forEach(row => {
      if (row.gid) {
        ...
        if (!groupClickMap.has(key)) groupClickMap.set(key, ++clickNum);
  ...
  function buildAnimationTab(list, header) {
    ...
    if (animSortView) {
      ...
      const rows = collectAnimationRows(slide);
      ...
      if (rows.length === 0) {
        ...
        rows.forEach(row => appendAnimationSummaryItem(list, row));
      }
      return;
    }
